...
package object barneshut {
  ...
  extends Quad {
    val mass = bodies.map(_.mass).sum : Float
    val (massX, massY) = ((bodies.map(b => b.x * b.mass).sum)/mass : Float, (bodies.map(b => b.y * b.mass).sum)/mass : Float)
    val total: Int = bodies.size
    def insert(b: Body): Quad = {
      if (size > minimumSize) {
        val quad = size / 4 : Float
        val half = size / 2 : Float
        var f = Fork(Empty(centerX - quad,centerY - quad,half),
             Empty(centerX + quad,centerY - quad,half),
             Empty(centerX - quad,centerY + quad,half),
             Empty(centerX + quad,centerY + quad,half))
        for (x <- b+:bodies) {
          f = f.insert(x) // May need to think about parallel it
        }
        //(b+:bodies).map(f = f.insert(_))
        f
      } else {
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def combine(that: SectorMatrix): SectorMatrix = {
      for (i <- 0 until matrix.size) matrix.update(i, matrix(i).combine(that.matrix(i)))
      this
    }
